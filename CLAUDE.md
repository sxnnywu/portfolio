...
- Clouds hang past the bottom of whatever holds them and rely on their bottom mask to fade out. Inside a clipped panel they can be cut at *either* end, so those take `fade="both"`. Do not clip a section that contains them unless its clouds are masked *and* the clip lands in the faded part: the sky band used to cut its tallest cloud at 68% opacity, which read as a straight line across the page. The Home hero is the reference — it does not clip at all.
