...
- Motion: `Typed` reveals hero and page titles a character at a time by stepping a clip-path, never by appending text. The text ships whole in the HTML, the element keeps its final width so a centred line cannot jitter, and the headline is still the LCP paint. There is deliberately no caret: glyph widths vary in a proportional face, so a caret cannot track the character edge.
